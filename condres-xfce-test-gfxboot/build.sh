...
make_syslinux() {
    ...
    cp ${script_path}/isolinux/gfxboot.cfg ${work_dir}/iso/${install_dir}/boot/syslinux/
...
# Build ISO
make_iso() {
    mkarchiso ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -o "${out_dir}" iso "condres_os-19.01-gfxboot-${iso_name}.iso"
}

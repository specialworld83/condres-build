...
make_syslinux() {
    ...
    gzip -c -9 ${work_dir}/${arch}/airootfs/usr/lib/modules/*-Condres-macbook/modules.alias > ${work_dir}/iso/${install_dir}/boot/syslinux/hdt/modalias.gz
}
...
# Build ISO
make_iso() {
    mkarchiso ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -o "${out_dir}" iso "condres_os_macbook-19.05-${iso_name}.iso"
}

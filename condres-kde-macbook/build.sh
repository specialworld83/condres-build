...
make_setup_mkinitcpio() {
    ...
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux-macbook -g /boot/archiso.img' run
}
...
make_boot() {
    ...
    cp ${work_dir}/${arch}/airootfs/boot/vmlinuz-linux-macbook ${work_dir}/iso/${install_dir}/boot/${arch}/vmlinuz-linux-macbook
}
...
make_syslinux() {
    ...
    gzip -c -9 ${work_dir}/${arch}/airootfs/usr/lib/modules/*-macbook-dirty/modules.alias > ${work_dir}/iso/${install_dir}/boot/syslinux/hdt/modalias.gz
}
...
make_efiboot() {
    ...
    cp ${work_dir}/iso/${install_dir}/boot/${arch}/vmlinuz-linux-macbook ${work_dir}/efiboot/EFI/archiso/vmlinuz-linux-macbook.efi
...
# Build ISO
make_iso() {
    mkarchiso ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -o "${out_dir}" iso "condres_os_macbook-19.03_beta-${iso_name}.iso"
}

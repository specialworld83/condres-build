...
make_syslinux() {
    ...
    gzip -c -9 ${work_dir}/${arch}/airootfs/usr/lib/modules/*-Condres/modules.alias > ${work_dir}/iso/${install_dir}/boot/syslinux/hdt/modalias.gz
}
...
# Build ISO
make_iso() {
    mkarchiso ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -o "${out_dir}" iso "condres_os-19.07-${iso_name}.iso"
}

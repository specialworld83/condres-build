...
set -e -u
edition=cli-installer
iso_name=cli-installer
iso_label=Condres
...
make_syslinux() {
    ...
    gzip -c -9 ${work_dir}/${arch}/airootfs/usr/lib/modules/*-CONDRES-dirty/modules.alias > ${work_dir}/iso/${install_dir}/boot/syslinux/hdt/modalias.gz
}
...
# Build ISO
make_iso() {
    mkarchiso ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -o "${out_dir}" iso "condres_os-19.03.1-${iso_name}.iso"
}
